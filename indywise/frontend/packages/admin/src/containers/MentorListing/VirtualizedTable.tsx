...
export default function VirtualizedTable() {
  const { mentorList } = React.useContext(MentorContext);

  if (mentorList?.length && mentorList.length > 0) {
    mentorList.sort(function (a: any, b: any) {
  ...
  const requestSearch = (searchedVal: string, type: string) => {
    let filteredRows;
    if (type === 'fullName') {
      filteredRows = mentorList.filter((row: any) => {
            ...
            {(rowsPerPage > 0
              ...
              <TableRow key={data.ID}>
                <TableCell align="center" component="th" scope="row">
                  <Link className={classes.link} to={`/mentor-list/${data.ID}`}>
                    {index}
                  </Link>
                </TableCell>
