...
export default function VirtualizedTable() {
  ...
  courseList.sort(function (a: any, b: any) {
  ...
  const requestSearch = (searchedVal: string, type: string) => {
    let filteredRows;
    if (type === 'title') {
      filteredRows = courseList.filter((row: any) => {
  ...
  return (
    ...
    >
      ...
      >
        <SearchBar
          value={searchedName}
          onChange={(searchVal) => requestSearch(searchVal, 'title')}
          onCancelSearch={() => cancelSearch('title')}
          placeholder="Title"
        />
        <SearchBar
            ...
            {(rowsPerPage > 0
              ...
              <TableRow key={data.ID}>
                <TableCell align="center" component="th" scope="row">
                  <Link className={classes.link} to={`/course-list/${data.ID}`}>
                    {index}
                  </Link>
                </TableCell>
