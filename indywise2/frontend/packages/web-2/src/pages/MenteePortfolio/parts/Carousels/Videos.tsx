...
const Videos: React.FC<{
  ...
  const handlePhotoSubmission = async (e: any) => {
    ...
    for (let i = 0; i < e.target.files.length; i++) {
      formData.append(
        `photo${(i + insertionIndex) % 10 === 0 ? 10 : (i + insertionIndex) % 10
        }`,
        e.target.files[i]
      );
    }
      ...
      uploadPromises.push(
        Axios.post(
          `${baseURL}/portfolios/portfolio/photo/presigned-url/${ID}`,
          {
            ...
            photoName: `photo${(i + insertionIndex) % 10 === 0 ? 10 : (i + insertionIndex) % 10
              }`
          }
        )
      );
            ...
            <div style={{ width: '100%' }}>
              {video?.mentee?.length > 0 && (
                <span
                  ...
                  onClick={() => {
                    Axios.delete(
                      `${baseURL}/portfolios/portfolio/videos?type=mentee`
                    ).then((res) => {
