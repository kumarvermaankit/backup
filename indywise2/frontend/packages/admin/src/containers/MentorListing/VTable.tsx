...
export default function VirtualizedTable() {
  const { updatedMentorList } = React.useContext(UpdatedMentorContext);

  if (updatedMentorList?.length && updatedMentorList.length > 0) {
    updatedMentorList.sort(function (a: any, b: any) {
  ...
  const requestSearch = (searchedVal: string, type: string) => {
    let filteredRows;
    if (type === 'fullName') {
      filteredRows = updatedMentorList.filter((row: any) => {
  ...
  return (
    ...
    >
      ...
      >
        <SearchBar
          value={searchedName}
          onChange={(searchVal) => requestSearch(searchVal, 'fullName')}
          onCancelSearch={() => cancelSearch('fullName')}
              ...
              <TableRow key={data.ID}>
                <TableCell align="center" component="th" scope="row">
                  <Link
                    ...
                    to={`/updated/mentor-list/${data.ID}`}
                  >
                    {index}
                  </Link>
                </TableCell>
