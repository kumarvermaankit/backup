...
const CertificationsForm: React.FC = () => {
  ...
  const addcertifications = () => {
    ...
    if (
      ...
      temp?.studiesAndCertification?.certifications?.length === 0
    )
      temp['studiesAndCertification'] = {
        certifications: [],
        education: temp?.studiesAndCertification?.education || []
      };
    ...
    temp.studiesAndCertification.certifications.push({
      ...
      description: '',
      certificate: { linkToCertificate: '', fileName: '' }
    });
  ...
  return (
    ...
    >
      ...
      portfolioData?.studiesAndCertification?.certifications &&
      portfolioData?.studiesAndCertification?.certifications?.length > 0
        ? portfolioData?.studiesAndCertification?.certifications.map(
                ...
                <Container key={index}>
                  ...
                  <Flex style={{ justifyContent: 'space-between' }}>
                    <TextField
                      ...
                      onChange={(e) =>
                        nestedHandleChange(e, index, 'linkToCertificate')
                      }
                      value={certficate.certificate.linkToCertificate || ''}
                      style={{ marginRight: '2vw', width: '50%' }}
                    />
                    <TextField
                      label="File Name"
                      variant="outlined"
                      name="fileName"
                      onChange={(e) => nestedHandleChange(e, index, 'fileName')}
                      value={certficate.certificate.fileName || ''}
                      style={{ marginRight: '2vw', width: '50%' }}
                    />
                  </Flex>
                </Container>
