...
const WantsInternshipInForm: React.FC<ISkillsFormProps> = ({ index }) => {
  const { removeRow, variables, handleValueChange } = React.useContext(
    MenteeContext
  );

  const { wantsInternshipIn } = variables;
  const { skill = '', category = '' } = wantsInternshipIn[index];
    ...
    <Line>
      <Flex>
        ...
        <Tooltip title="Remove skill">
          <Remove onClick={(e) => removeRow(e, index, 'wantsInternshipIn')} />
        </Tooltip>
      </Flex>
      <Flex>
        <TextField
          ...
          variant="outlined"
          name="skill"
          onChange={(e) => handleValueChange(e, index, 'wantsInternshipIn')}
          value={skill || ''}
...
export default WantsInternshipInForm;
