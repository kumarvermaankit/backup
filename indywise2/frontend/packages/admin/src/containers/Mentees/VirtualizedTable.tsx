...
export default function VirtualizedTable() {
  const { menteeList } = React.useContext(MenteeContext);
  menteeList.sort(function (a: any, b: any) {
    var nameA = a.fullName.trim().toUpperCase();
    var nameB = b.fullName.trim().toUpperCase();
    if (nameA < nameB) {
      return -1;
    }
    if (nameA > nameB) {
  ...
  const requestSearch = (searchedVal: string, type: string) => {
    let filteredRows;
    if (type === 'fullName') {
      filteredRows = menteeList.filter((row: any) => {
            ...
            {(rowsPerPage > 0
              ...
              <TableRow key={data.ID}>
                <TableCell align="center" component="th" scope="row">
                  <Link className={classes.link} to={`/mentee-list/${data.ID}`}>
                    {index}
                  </Link>
                </TableCell>
